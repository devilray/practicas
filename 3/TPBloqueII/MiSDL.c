...
void   MiSDL_PintarImagen(MiSDL _miSDL, int coordenadaX, int coordenadaY, int anchura,int altura, char * rutaImagen)
{
    MiSDLAp miSDL = _miSDL;
    SDL_Surface* imagen = SDL_LoadBMP(rutaImagen);
    SDL_Rect imagenDimensiones = {coordenadaX,coordenadaY,anchura, altura};
    SDL_BlitSurface(imagen, anchura*altura, miSDL->surface, &imagenDimensiones); //Quizas haya que quitar altura
